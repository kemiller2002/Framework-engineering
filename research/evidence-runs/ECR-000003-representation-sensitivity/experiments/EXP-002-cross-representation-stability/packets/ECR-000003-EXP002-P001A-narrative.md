...
recognition_condition: narrative
blinded_artifact_id: BR-EXP002-P001

blinded_procedural_content:

An unresolved issue appears.

Several possible paths are generated.

An additional input is gathered that helps distinguish among the paths.

The paths are reassessed using that input.

If the distinction remains too weak, the process loops and gathers more differentiating input.

The process ends when one path is selected or further continuation is no longer justified.
