...
recognition_condition: domain_variant_technical
blinded_artifact_id: BR-EXP003-P001

blinded_procedural_content:

A system has an unresolved fault.

Candidate causes are listed.

Diagnostic signals are gathered.

Candidate causes are tested against the signals.

The process loops if the cause remains unresolved.

The process ends when a cause is accepted or further diagnosis is not justified.
