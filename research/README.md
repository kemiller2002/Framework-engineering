# Research

Status: Working draft

Research contains hypotheses, experiments, evidence, corpora, active questions, deferred insights, and failed hypotheses.

Research is exploratory.

Research artifacts should clearly state status:

- Hypothesis
- Under Investigation
- Provisionally Supported
- Foundation Candidate
- Superseded
- Failed

Operating system:

- [Research Operating System](operating-system/README.md)
- [Hypothesis Registry](operating-system/hypothesis-registry.md)
- [Research Operating Manual](operating-system/research-operating-manual.md)
- [Research Constitution](operating-system/research-constitution.md)
- [Research Dependency Graph](operating-system/research-dependency-graph.md)
- [Evidence Decision Records](operating-system/edr/README.md)
- [Research Queue](operating-system/research-queue/README.md)

Product architecture:

- [Clarity Object Model v0.1](product-architecture/clarity-object-model-v0.1.md)
- [Uncertainty to Action Flow](product-architecture/uncertainty-to-action-flow.md)
- [Research and Product Boundary](product-architecture/research-product-boundary.md)

Experiments:

- [FE-011A LLM Blind Pilot](experiments/FE-011A-llm-blind-pilot/README.md)
- [FE-012A Blind Primitive Reasoning Extraction](experiments/FE-012A-blind-primitive-extraction/README.md)
- [FE-012B Primitive Grammar Synthesis Experiment](experiments/FE-012B-primitive-synthesis/README.md)
- [FE-012C Manual Multi-Model Replication Kit](experiments/FE-012C-manual-replication/README.md)
- [FE-012C Repeatability Run 001](experiments/FE-012C-repeatability-run-001/README.md)
- [FE-013 Procedural Language Hypothesis](experiments/FE-013-procedural-language-hypothesis/README.md)

Evidence runs:

- [ECR-000001 Multi-Layer Procedural Representation](evidence-runs/ECR-000001-multi-layer-procedural-representation/README.md)
- [ECR-000001 Limited Review Workspace](evidence-runs/ECR-000001-multi-layer-procedural-representation/review/README.md)
- [ECR-000002 Recognition Sensitivity](evidence-runs/ECR-000002-recognition-sensitivity/README.md)
- [ECR-000003 Representation Sensitivity](evidence-runs/ECR-000003-representation-sensitivity/README.md)

FE-013 manual execution:
- Use `run-instructions.md` and `operator-checklist.md` in `research/experiments/FE-013-procedural-language-hypothesis/` for manual console execution across GPT-5, Claude, and Gemini.

FE-012C reporting:
- Generated reports are written under `research/experiments/FE-012C-manual-replication/reports/`.

FE-012C repeatability:
- Repeatability runs reuse the locked packet instrument and must not expose prior outputs to participating models.
- Dataset comparison outputs for repeatability run 001 are generated under `research/experiments/FE-012C-repeatability-run-001/comparison/generated/`.

Validation:

- [Framework Engineering Validation Program](validation/validation-program.md)
- [Adversarial Review Protocol](validation/adversarial-review-protocol.md)
- [Competing Theory Challenge](validation/competing-theory-challenge.md)
- [Independent Replication Plan](validation/independent-replication-plan.md)
- [Claim Review Template](validation/claim-review-template.md)
- [Validation Roadmap](validation/validation-roadmap.md)

Theory:

- [Theory of Framework Engineering v0.1](theory/theory-of-framework-engineering-v0.1.md)
- [Emergent Design Laws v0.1](theory/emergent-design-laws-v0.1.md)
- [Grammar Hypotheses v0.1](theory/grammar-hypotheses-v0.1.md)
- [Falsification Criteria](theory/falsification-criteria.md)
- [Theory Traceability Matrix v0.1](theory/theory-traceability-matrix-v0.1.md)
- [Evidence To Theory Map](theory/evidence-to-theory-map.md)
- [Competing Explanations](theory/competing-explanations.md)
- [Confidence Assessment](theory/confidence-assessment.md)
- [Future Predictions](theory/future-predictions.md)
- [Research Program Status](theory/research-program-status.md)
